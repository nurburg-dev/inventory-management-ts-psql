...
const categories = [
  'Electronics', 'Furniture', 'Office Supplies', 'Clothing', 'Books',
  'Sports', 'Home & Garden', 'Automotive', 'Health & Beauty', 'Toys',
  'Music & Movies', 'Food & Beverage', 'Tools & Hardware', 'Pet Supplies', 'Jewelry'
];

// Product names for variety
const productNames = [
  'Laptop', 'Mouse', 'Keyboard', 'Monitor', 'Chair', 'Desk', 'Phone', 'Tablet',
  'Headphones', 'Speaker', 'Camera', 'Printer', 'Scanner', 'Router', 'Cable',
  'Charger', 'Case', 'Stand', 'Adapter', 'Battery', 'Memory Card', 'Drive',
  'Notebook', 'Pen', 'Pencil', 'Paper', 'Folder', 'Binder', 'Stapler', 'Clips'
];
...
export function setupItems() {
  ...
  if (response.status !== 201) {
    console.error(`Failed to create item: ${response.status} - ${response.body}`);
  }
}
...
export function loadTest() {
  ...
  const response = http.get(`${API_HOST}/api/items/category/${encodeURIComponent(category)}/top`, params);
  ...
  if (!checkResult) {
    console.error(`Failed request for category ${category}: ${response.status} - ${response.body.slice(0, 200)}`);
  }
